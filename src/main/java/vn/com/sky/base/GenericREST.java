...
public class GenericREST {
	...
	protected Mono<ServerResponse> error(Throwable e) {
		e.printStackTrace();
		return Mono.just(((Exception) e).getMessage()).flatMap(errorRes -> ServerResponse.badRequest()
				.contentType(MediaType.APPLICATION_JSON).bodyValue(e.getMessage()));
	}
	...
	protected <T extends GenericEntity> Mono<T> saveEntity(ReactiveCrudRepository<T, Long> repo, T entity,
			Long userId) {
	...
	protected <T extends GenericEntity> Mono<T> updateEntity(ReactiveCrudRepository<T, Long> repo, T entity,
			Long userId) {
	...
	protected <T extends GenericEntity> Mono<T> softDeleteEntity(ReactiveCrudRepository<T, Long> repo, T entity,
			Long userId) {
...
//	protected <T extends GenericEntity> Mono<List<T>> saveManyRelation(OneToOneRepo<T> repo, Long mainId,
//			ArrayList<Long> subIds, SaveOneToOneRelation<T> saveOneToOne, ServerRequest request) {
//		if (subIds == null) {
//			return Mono.empty();
//		} else {
//			return Flux.fromIterable(subIds).flatMap(subId -> {
//				return repo.findRelation(mainId, subId).flatMap(found -> {
...
//	}
//
//	protected <T extends GenericEntity> Mono<List<Void>> deleteManyRelation(OneToOneRepo<T> repo, Long mainId,
